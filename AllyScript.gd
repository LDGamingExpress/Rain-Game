...
const SPEED = 100.0 # Speed of the ally
var Firerate = 0.2 # How long the ally has to wait between shots
var Damage = 1 # Damage of the ally
var Health = 3 # Health of the ally
var FireReady = 1 # 1 means the ally is ready to fire, 0 means the ally needs to wait to fire
var Movement = "Random" # Current movement type
# Random - ally moves around randomly
# Targeted - ally goes after a particular ally
# Shooting - ally is stationary to shoot
var Target = position # position ally is moving to
...
func _ready():
	...
	# Sets the target to random position near the ally

func _physics_process(delta):
	if Health > 0: # Only lets the ally move or shoot if alive
		...
		if len(HostilesFound) > 0 and Movement != "Targeted" and Movement != "Shooting": # Checks that the ally has found hostiles and is not currently targeting anyone
		...
		if Movement == "Targeted": # Checks if already targeting someone
			if !is_instance_valid(Target): # Checks if target no longer exists
				...
				# Checks if the ally is within a certain distance of their target
		...
		if len(HostilesFound) == 0: # Checks if there are no hostiles in range
			if Movement == "Random": # Checks if the ally is already moving randomly
				if (abs(position.x - Target.x) + abs(position.y - Target.y)) < 15 or sqrt(pow((position.x - LastPos.x),2) + pow((position.y - LastPos.y),2)) < 0.5:
					# Checks if the ally has already reached their target or if they are stuck
					Target = position + Vector2(rng.randi_range(-50,50),rng.randi_range(-50,50)) - 20 * Vector2(cos(rotation),sin(rotation))
					# Sets a new target position
			else:
				Target = position + Vector2(rng.randi_range(-50,50),rng.randi_range(-50,50))
		...
		if Movement == "Shooting":
			if !is_instance_valid(Target): # Checks if target no longer exists
				...
				NewBul.Team = "Allies" # Sets team to match group
				NewBul.Damage = Damage # Gives bullet ally's damage
				get_parent().add_child(NewBul) # Creates bullet as child of parent
			else:
				look_at(Vector2(Target.position.x,Target.position.y)) # Looks at target
		LastPos = position
		move_and_slide() # Moves ally based on velocity
...
func _on_detection_area_body_entered(body): # Activates when an object enters the detection area
	if body.is_in_group("Enemies"): # Checks if object is a hostile
